...
const traverse = (
  ...
  while (iTraverses < 4 && jTraverses < 4) {
    if (!crossword[i + iTraverses * iTraverseMultiplier]) break;
    maybeMasString +=
      crossword[i + iTraverses * iTraverseMultiplier][
        j + jTraverses * jTraverseMultiplier
      ];
    iTraverses++;
    jTraverses++;
  }
  if (maybeMasString === "MAS") return true;
  return false;
};
